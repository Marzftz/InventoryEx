...
router.get('/Solpendientes', async(req, res)=>{
    ...
    const solicitudes = await db.query(`SELECT idSolicitud_de_Producto, productos.InvCodigo_Producto, productos.InvNombre_Producto, SolCantidad_Producto, date_format(SolFecha_Solicitud, "%d-%m-%Y %h:%i:%s %p") AS SolFecha_Solicitud, usuario.UsuNombre, productos.InvPrecio_Unitario
...
router.post('/rechazarSolicitud', async(req, res) => {
  ...
  var fecha = (new Date(Date.now() - tzoffset)).toISOString().slice(0, -5).replace ("T", " ");
    ...
    db.query(consulta, async (err, result) => {        
        ...
        if (result.length != 0) {
          ...
          const query2 = await db.query(`UPDATE solicitud_de_producto SET  SolFecha_Cierre_Solicitud = '${fecha}' where idSolicitud_de_Producto = ${parseInt(idSolicitud)};`);         
    ...
    db.query(consulta, async (err, result) => {        
        ...
        if (result.length != 0) {
            ...
            if (canExistente[0].InvCantidad > canSolicitud) {
                ...
                const query3 = await db.query(`UPDATE solicitud_de_producto SET  SolFecha_Cierre_Solicitud = '${fecha}' where idSolicitud_de_Producto = ${parseInt(idSolicitud)};`);         
                //Se realiza el llamado a la BD
                await db.query(query, (err, result) => {
                    if (err) {
                    //Si existe un error se imprime en consola
